---
title: "Poder e ANOVA: uma simulação de desempenho dos delineamentos DIC e BIC"
author: "Cristiane Zhu e Júlia Zorzo Ferreira"
format:
  html:
    theme: journal
    toc: true
    toc-location: left
    toc-depth: 3
    toc-title: "Sumário"
    number-sections: true
    smooth-scroll: true

editor: visual
---

# Contextualização

Entre os delineamentos mais utilizados no planejamento de experimentos estão o **delineamento inteiramente casualizado (DIC)** e o **delineamento em blocos completos casualizados (BIC)**. Ambos avaliam os efeitos de tratamento - o que se aplica nas unidades experimentais - por meio da análise de variância (ANOVA). A fim de conduzir um experimento adequadamente, é natural o surgimento de dúvidas em relação à escolha do melhor delineamento, sendo essencial, portanto, compreender o desempenho de cada um dado o contexto específico.

No delineamento inteiramente casualizado, toda variação não explicada pelos tratamentos é absorvida pelo erro. Já o BIC introduz uma estrutura de blocos que, quando relevante, retira do erro uma parcela da variabilidade — reduzindo o quadrado médio dele e aumentando a estatística F do teste. Por essa razão, o BIC pode ter maior poder que o DIC. Porém, quando o efeito de bloco é fraco ou inexistente, o BIC perde graus de liberdade sem qualquer ganho correspondente, penalizando o teste. Dessa forma, nem sempre o delineamento em blocos é a melhor escolha para o determinado experimento, já que não apresenta melhor eficácia que o delineamento inteiramente casualizado em todos os casos.

O poder do teste — probabilidade de rejeitar a hipótese nula quando ela é falsa — depende simultaneamente da magnitude dos efeitos de tratamento, da variabilidade do erro e da relevância da estrutura de blocos. Como esses fatores raramente são conhecidos a priori, a simulação computacional permite investigar como o poder se comporta em múltiplas replicações do experimento com diferenciação dos blocos para um cenário fixado, o qual pode ser alterado de acordo com o que se pretende observar nos resultados de desempenho.

Diante disso, o objetivo deste trabalho é comparar o poder empírico do DIC e do BIC por meio de simulações de Monte Carlo. Para tanto, fixam-se parâmetros e variam-se as intensidades dos efeitos de bloco. Por meio dessas análises, evidencia-se principalmente o ponto em que o delineamento em blocos completos passa a ser preferível ao inteiramente casualizado, para o determinado experimento, ilustrando o resultado das simulações juntamente às demais representações gráficas incluídas.

# Teoria

## Modelos DIC e BIC

### Modelo DIC

$$
Y_{ij} = \mu + \tau_i + \varepsilon_{ij}, \quad i = 1, \dots, k;\; j = 1, \dots, r
$$

Onde:

-   $Y_{ij}$: observação do $i$-ésimo tratamento na $j$-ésima repetição\
-   $\mu$: média geral\
-   $\tau_i$: efeito do $i$-ésimo tratamento\
-   $\varepsilon_{ij} \sim N(0, \sigma^2)$: erro aleatório

### Modelo BIC

$$
Y_{ij} = \mu + \tau_i + \beta_j + \varepsilon_{ij}, \quad i = 1, \dots, k;\; j = 1, \dots, b
$$

Onde:

-   $Y_{ij}$: observação do $i$-ésimo tratamento no $j$-ésimo bloco\
-   $\mu$: média geral\
-   $\tau_i$: efeito do $i$-ésimo tratamento\
-   $\beta_j$: efeito do $j$-ésimo bloco\
-   $\varepsilon_{ij} \sim N(0, \sigma^2)$: erro aleatório

#### Suposições a serem satisfeitas

$\varepsilon_{ij}$ independentes

$$
\varepsilon_{ij} \sim N(0,\sigma^2)
$$

$$
\sum_{i=1}^{t}\tau_i = 0
$$

$$
\sum_{i=1}^{j}\beta_j = 0
$$

### ANOVA: Análise de Variância

A ANOVA decompõe a variação total dos dados em fontes identificáveis, avaliando se a variação atribuída aos tratamentos supera o ruído dos resíduos. Para isso, são calculadas somas de quadrados e quadrados médios, utilizados na obtenção matemática da estatística de teste F, como ilustrado nas tabelas:

```{r}
#| echo: false
tabela_dic <- data.frame(
  `Fonte de Variação` = c("Tratamento", "Resíduo", "Total"),
  `GL`  = c("t-1", "N-1", "N-t"),
  `SQ`  = c("SQTrat", "SQRes", "SQTotal"),
  `QM`  = c("QMTrat", "QMRes", "-"),
  `F`   = c("QMTrat/QMRes", "-", "-"),
  check.names = FALSE
)

tabela_dbc <- data.frame(
  `Fonte de Variação` = c("Tratamento", "Bloco", "Resíduo", "Total"),
  `GL`  = c("t-1", "b-1", "(t-1)(b-1)", "tb-1"),
  `SQ`  = c("SQTrat", "SQBloco", "SQRes", "SQTotal"),
  `QM`  = c("QMTrat", "QMBloco", "QMRes", "-"),
  `F`   = c("QMTrat/QMRes", "QMBloco/QMRes", "-", "-"),
  check.names = FALSE
)
```

```{r}
#| echo: false

knitr::kable(
  tabela_dic,
  caption = "Tabela Anova para DIC"
)
```

```{r}
#| echo: false

knitr::kable(
  tabela_dbc,
  caption = "Tabela ANOVA para BIC"
)
```

## Teste de Hipóteses

### Efeito de tratamento: DIC e BIC

#### Hipótese nula

Os efeitos de tratamento são iguais

$$
H_0: \tau_1 = \tau_2 = \tau_3 = \tau_4 = 0
$$

#### Hipótese alternativa

Pelo menos um efeito de tratamento difere de zero

$$
H_1: \exists \, i \text{ tal que } \tau_i \neq 0
$$
Como se deseja avaliar o poder do teste, a probabilidade de rejeitar a hipótese nula dado que ela é falsa, essa hipótese é rejeitada, portanto considera-se que pelo menos um efeito de tratamento difere de zero.

No caso do BIC, as mesmas hipóteses são consideradas para o efeito do bloco:

### Efeito de bloco: BIC

#### Hipótese nula

Os efeitos de bloco são iguais

$$
H_0: \beta_1 = \beta_2 = \beta_3 = \beta_4 = 0 
$$

#### Hipótese alternativa

Pelo menos um efeito de bloco difere de zero
$$
H_1: \exists \, j \text{ tal que } \beta_j \neq 0
$$

# Simulações

```{r}
#| echo: false
# Bibliotecas
library(ggplot2)
library(knitr)
library(tidyr)
```

## Fixando valores

```{r}
tratamentos <- 4
blocos <- 4
n_obs <- tratamentos * blocos

trat  <- rep(1:tratamentos, times = blocos)
bloco <- rep(1:blocos, each = tratamentos)

mu <- 100

# tau arbitrário com efeitos significativos:
tau <- c(0, 4, 8, -12) 
```

Em que n_obs é o número de observações, mu é a média geral das observações e tau o efeito de tratamento, necessariamente significativo, já que as diferenças de poder derivam de uma hipótese alternativa verdadeira.

### Escolha da variância

```{r}
# escolha de uma variância arbitrária
sigma2 <- 126
```

Como a variância dos resíduos é superior à variância dos efeitos de tratamento (56), a detecção desses efeitos não é trivial. Ao mesmo tempo, essa razão não é suficientemente extrema — nem pequena a ponto de tornar o poder próximo de 1, nem grande a ponto de torná-lo próximo de 0 — configurando, portanto, um cenário de detecção moderada que favorece a diferenciação do poder empírico entre os delineamentos.

### Efeitos de bloco
Para comparar o poder entre os modelos DIC e BIC, três cenários de intensidade dos efeitos de bloco são propostos: fraco, intermediário e forte.

#### Normalidade
É importante sinalizar que considera-se o efeito do j-ésimo bloco como uma variável normal, garantindo a suposição inicialmente apontada para os blocos. Sua distribuição é dada por:

$$
\beta_{j} \sim N(0,s^2)
$$
Essa escolha é justificada pelo princípio da máxima entropia: a distribuição normal é a menos restritiva, ou seja, qualquer outra distribuição introduziria suposições adicionais às já definidas pelo modelo. E, também, pelo Teorema Central do Limite: à medida que o número de replicações na simulação  aumenta, a distribuição amostral do poder empírico converge para uma normal independentemente da distribuição geradora dos betas.

#### Função para gerar betas aleatórios
```{r}
gera_beta <- function(blocos, desvio, seed = 123) {
  set.seed(seed)
  beta_orig <- rnorm(blocos, 0, sd = desvio)
  beta_orig - mean(beta_orig)
}
```

#### Desvios
Em cada cenário, considera-se uma porcentagem da variância total como a variância do bloco e são gerados os vetores de beta a partir desses desvios.

Além disso, são definidas as variâncias dos erros como consequência de cada porcentagem considerada para os blocos.
```{r}
# Fraco - 8%
sigma2_bloco_fraco <- 0.08 * sigma2  
sigma2_erro_fraco  <- 0.92 * sigma2  

beta_fraco <- gera_beta(blocos, desvio = sqrt(sigma2_bloco_fraco))

# Intermediário — 40%
sigma2_bloco_inter <- 0.40 * sigma2 
sigma2_erro_inter  <- 0.60 * sigma2

beta_inter <- gera_beta(blocos, desvio = sqrt(sigma2_bloco_inter))

# Forte — 70%
sigma2_bloco_forte <- 0.70 * sigma2
sigma2_erro_forte  <- 0.30 * sigma2  

beta_forte <- gera_beta(blocos, desvio = sqrt(sigma2_bloco_forte))


cat("Vetor de valores para os betas:\n");
cat("Efeito fraco:\n"); (beta_fraco)
cat("Efeito intermediário:\n"); (beta_inter)
cat("Efeito forte:\n"); (beta_forte)

```

## Testes de poder

### Efeito de bloco fraco

#### Cálculo do poder

```{r}
set.seed(123)

sim_bfraco <- 
  replicate(1000, {
  erro  <- rnorm(n_obs, 0, sqrt(sigma2_erro_fraco))
  y     <- mu + tau[trat] + beta_fraco[bloco] + erro
  p_bic <- summary(aov(y ~ factor(trat) + factor(bloco)))[[1]]["factor(trat)", "Pr(>F)"]
  p_dic <- summary(aov(y ~ factor(trat)))[[1]]["factor(trat)",  "Pr(>F)"]
  c(bic = p_bic < 0.05, dic = p_dic < 0.05,
    p_bic = p_bic, p_dic = p_dic)
})

cat("Poder para efeito de bloco fraco:\n");
cat("DIC:\n"); (poder_dic_bfraco <- mean(sim_bfraco["dic", ]))
cat("BIC:\n"); (poder_bic_bfraco <- mean(sim_bfraco["bic", ]))
```

No cenário de bloco fraco, os dois modelos apresentam poder similar, com o DIC levemente superior ao BIC. Quando o efeito de bloco é irrelevante, o BIC não se beneficia do controle local que propõe e a perda de graus de liberdade para estimar o efeito de bloco prejudica o seu poder, o que explica esse resultado.

#### Distribuição p-valor

```{r}
pvalores_fraco <- data.frame(
  p      = c(sim_bfraco["p_dic", ], sim_bfraco["p_bic", ]),
  Modelo = rep(c("DIC", "BIC"), each = 1000)
)

pvalores_fraco$Modelo <- factor(pvalores_fraco$Modelo, levels = c("DIC", "BIC"))

ggplot(pvalores_fraco, aes(x = p, fill = Modelo)) +
  geom_histogram(alpha = 0.6, bins = 30, position = "identity") +
  geom_vline(xintercept = 0.05, linetype = "dashed") +
  scale_fill_manual(values = c(BIC = "#110C5E", DIC = "#029C57")) +
  labs(x = "p-valor", y = "Número de simulações") +
  facet_wrap(~Modelo) +
  theme_minimal(base_family = "serif")
```

O gráfico mostra que, no cenário de bloco fraco, aproximadamente 40% dos p-valores de ambos os modelos ficaram abaixo de alpha = 0,05, indicando poder moderado para os dois delineamentos, como verificado.

### Efeito de bloco intermediário

#### Cálculo do poder

```{r}
#| echo: false
set.seed(123)

sim_binter <- 
  replicate(1000, {
  erro  <- rnorm(n_obs, 0, sqrt(sigma2_erro_inter))
  y     <- mu + tau[trat] + beta_inter[bloco] + erro
  p_bic <- summary(aov(y ~ factor(trat) + factor(bloco)))[[1]]["factor(trat)", "Pr(>F)"]
  p_dic <- summary(aov(y ~ factor(trat)))[[1]]["factor(trat)",  "Pr(>F)"]
  c(bic = p_bic < 0.05, dic = p_dic < 0.05,
    p_bic = p_bic, p_dic = p_dic)
})
cat("Poder para efeito de bloco intermediário:\n")
cat("DIC:\n"); (poder_dic_binter <- mean(sim_binter["dic", ]))
cat("BIC:\n"); (poder_bic_binter <- mean(sim_binter["bic", ]))
```

No cenário de bloco intermediário, o BIC apresenta poder consideravelmente superior ao DIC. Quando o efeito de bloco é moderado, o DIC absorve essa variação no resíduo, inflando o quadrado médio do erro e reduzindo a estatística F. Já o BIC isola o efeito de bloco antes de testar os tratamentos, reduzindo o resíduo e aumentando o poder do teste — evidenciando a vantagem do controle local quando a estrutura de blocos é relevante.

#### Distribuição p-valor

```{r}
#| echo: false
pvalores_inter <- data.frame(
  p      = c(sim_binter["p_dic", ], sim_binter["p_bic", ]),
  Modelo = rep(c("DIC", "BIC"), each = 1000)
)

pvalores_inter$Modelo <- factor(pvalores_inter$Modelo, levels = c("DIC", "BIC"))

ggplot(pvalores_inter, aes(x = p, fill = Modelo)) +
  geom_histogram(alpha = 0.6, bins = 30, position = "identity") +
  geom_vline(xintercept = 0.05, linetype = "dashed") +
  scale_fill_manual(values = c(BIC = "#110C5E", DIC = "#029C57")) +
  labs(x = "p-valor", y = "Número de simulações") +
  facet_wrap(~Modelo) +
  theme_minimal(base_family = "serif")
```

O gráfico mostra que, no cenário de efeito de bloco intermediário, aproximadamente 64% dos p-valores do BIC ficam abaixo do nível de significância alpha = 0,05 contra 41% no DIC. A concentração de p-valores pequenos é visivelmente maior para o BIC, confirmando que o controle local melhora substancialmente a capacidade de detectar o efeito de tratamento quando o bloco tem relevância moderada.

### Efeito de bloco forte

#### Cálculo do poder

```{r}
#| echo: false
set.seed(123)

sim_bforte <- 
  replicate(1000, {
  erro  <- rnorm(n_obs, 0, sqrt(sigma2_erro_forte))
  y     <- mu + tau[trat] + beta_forte[bloco] + erro
  p_bic <- summary(aov(y ~ factor(trat) + factor(bloco)))[[1]]["factor(trat)", "Pr(>F)"]
  p_dic <- summary(aov(y ~ factor(trat)))[[1]]["factor(trat)",  "Pr(>F)"]
  c(bic = p_bic < 0.05, dic = p_dic < 0.05,
    p_bic = p_bic, p_dic = p_dic)
})

cat("Poder para efeito de bloco forte:\n")
cat("DIC:\n"); (poder_dic_bforte <- mean(sim_bforte["dic", ]))
cat("BIC:\n"); (poder_bic_bforte <- mean(sim_bforte["bic", ]))
```
No cenário de bloco forte, a diferença entre os modelos é bem significativa, o BIC atinge poder de 0,928, detectando o efeito de tratamento em quase todas as simulações, enquanto o DIC cai para 0,343 — menos da metade do poder do BIC. Quando o efeito de bloco é grande, o DIC absorve toda essa variação no resíduo, inflando o quadrado médio do erro a ponto de tornar o teste de tratamento quase incapaz de detectar diferenças reais. O BIC, ao isolar e remover o efeito de bloco antes de testar os tratamentos, preserva o poder do teste como observado nos cenários anteriores.

#### Distribuição p-valor

```{r}
#| echo: false
pvalores_forte <- data.frame(
  p      = c(sim_bforte["p_dic", ], sim_bforte["p_bic", ]),
  Modelo = rep(c("DIC", "BIC"), each = 1000)
)

pvalores_forte$Modelo <- factor(pvalores_forte$Modelo, levels = c("DIC", "BIC"))

ggplot(pvalores_forte, aes(x = p, fill = Modelo)) +
  geom_histogram(alpha = 0.6, bins = 30, position = "identity") +
  geom_vline(xintercept = 0.05, linetype = "dashed") +
  scale_fill_manual(values = c(BIC = "#110C5E", DIC = "#029C57")) +
  labs(x = "p-valor", y = "Número de simulações") +
  facet_wrap(~Modelo) +
  theme_minimal(base_family = "serif")
```

O gráfico mostra que, no cenário de bloco forte, a maioria dos p-valores no modelo BIC se concentra abaixo de 0,05. Já para DIC, quase todos os valores estão acima de 0,05, ou seja, O DIC apresenta poder significantemente inferior ao BIC.

### Comparação das distribuições p-valor

```{r}
#| echo: false
pvalores_todos <- data.frame(
  p_valor = c(sim_bfraco["p_dic", ], sim_bfraco["p_bic", ],
              sim_binter["p_dic", ], sim_binter["p_bic", ],
              sim_bforte["p_dic", ], sim_bforte["p_bic", ]),
  modelo  = rep(c("DIC", "BIC"), times = 3, each = 1000),
  cenario = rep(c("Bloco fraco", "Bloco intermediário", "Bloco forte"),
                each = 2000)
)

pvalores_todos$cenario <- factor(pvalores_todos$cenario,
  levels = c("Bloco fraco", "Bloco intermediário", "Bloco forte")
)

pvalores_todos$modelo <- factor(pvalores_todos$modelo,
  levels = c("DIC", "BIC")
)

ggplot(pvalores_todos, aes(x = p_valor, fill = modelo)) +
  geom_histogram(alpha = 0.6, bins = 30, position = "identity") +
  geom_vline(xintercept = 0.05, linetype = "dashed") +
  facet_grid(cenario ~ modelo) +
  scale_fill_manual(values = c(BIC = "#110C5E", DIC = "#029C57")) +
  labs(x = "p-valor", y = "Número de simulações", fill = NULL) +
  theme_minimal(base_family = "serif") +
  theme(legend.position = "none")
```

### Análise de convergência

```{r}
#| echo: false
set.seed(123)

replicas <- c(100, 1000, 10000)

simula_replicas1 <- function(beta, replicas) {
  lapply(replicas, function(B) {
    sim <- replicate(B, {
      erro  <- rnorm(n_obs, 0, sqrt(sigma2_erro_fraco))
      y     <- mu + tau[trat] + beta[bloco] + erro
      
      p_bic <- summary(aov(y ~ factor(trat) + factor(bloco)))[[1]]["factor(trat)", "Pr(>F)"]
      p_dic <- summary(aov(y ~ factor(trat)))[[1]]["factor(trat)", "Pr(>F)"]
      
      c(bic = p_bic < 0.05, dic = p_dic < 0.05)
    })
    
    data.frame(
      replicacoes = B,
      poder_dic   = round(mean(sim["dic", ]),3),
      poder_bic   = round(mean(sim["bic", ]),3)
    )
  })
}

simula_replicas2 <- function(beta, replicas) {
  lapply(replicas, function(B) {
    sim <- replicate(B, {
      erro  <- rnorm(n_obs, 0, sqrt(sigma2_erro_inter))
      y     <- mu + tau[trat] + beta[bloco] + erro
      
      p_bic <- summary(aov(y ~ factor(trat) + factor(bloco)))[[1]]["factor(trat)", "Pr(>F)"]
      p_dic <- summary(aov(y ~ factor(trat)))[[1]]["factor(trat)", "Pr(>F)"]
      
      c(bic = p_bic < 0.05, dic = p_dic < 0.05)
    })
    
    data.frame(
      replicacoes = B,
      poder_dic   = round(mean(sim["dic", ]),3),
      poder_bic   = round(mean(sim["bic", ]),3)
    )
  })
}

simula_replicas3 <- function(beta, replicas) {
  lapply(replicas, function(B) {
    sim <- replicate(B, {
      erro  <- rnorm(n_obs, 0, sqrt(sigma2_erro_forte))
      y     <- mu + tau[trat] + beta[bloco] + erro
      
      p_bic <- summary(aov(y ~ factor(trat) + factor(bloco)))[[1]]["factor(trat)", "Pr(>F)"]
      p_dic <- summary(aov(y ~ factor(trat)))[[1]]["factor(trat)", "Pr(>F)"]
      
      c(bic = p_bic < 0.05, dic = p_dic < 0.05)
    })
    
    data.frame(
      replicacoes = B,
      poder_dic   = round(mean(sim["dic", ]),3),
      poder_bic   = round(mean(sim["bic", ]),3)
    )
  })
}

res_rep <- rbind(
  cbind(cenario = "Bloco fraco",         do.call(rbind, simula_replicas1(beta_fraco, replicas))),
  cbind(cenario = "Bloco intermediário", do.call(rbind, simula_replicas2(beta_inter, replicas))),
  cbind(cenario = "Bloco forte",         do.call(rbind, simula_replicas3(beta_forte, replicas)))
)
```

```{r}
#| echo: false
knitr::kable(
  res_rep[res_rep$cenario == "Bloco fraco", -1],
  caption = "Poder empírico do DIC e BIC — Bloco fraco"
)

knitr::kable(
  res_rep[res_rep$cenario == "Bloco intermediário", -1],
  caption = "Poder empírico do DIC e BIC — Bloco intermediário"
)

knitr::kable(
  res_rep[res_rep$cenario == "Bloco forte", -1],
  caption = "Poder empírico do DIC e BIC — Bloco forte"
)
```

Em todos os cenários, os valores de poder empírico são aproximados ao valor real conforme o número de réplicas aumenta. De 100 para 10000 replicações, a variação é pequena, o que sustenta o uso padrão de 1000 repetições como suficiente para estimativas e resultados confiáveis.

### Poder por cenário de bloco

```{r}
#| echo: false
tabela_long <- pivot_longer(
  data.frame(
    cenario = c("Bloco fraco", "Bloco intermediário", "Bloco forte"),
    DIC = c(poder_dic_bfraco,
        poder_dic_binter,
        poder_dic_bforte),
    BIC = c(poder_bic_bfraco,
        poder_bic_binter,
        poder_bic_bforte)
  ),
  cols      = c(DIC, BIC),
  names_to  = "modelo",
  values_to = "poder"
)

tabela_long$cenario <- factor(tabela_long$cenario,
  levels = c("Bloco fraco", "Bloco intermediário", "Bloco forte")
)

tabela_long$modelo <- factor(tabela_long$modelo, levels = c("DIC", "BIC"))

ggplot(tabela_long, aes(x = cenario, y = poder, fill = modelo)) +
  geom_col(position = "dodge", width = 0.6, alpha = 0.65) +
  geom_text(aes(label = scales::number(poder, accuracy = 0.001)),
            position = position_dodge(width = 0.6),
            vjust = -0.4, size = 3.5) +
  scale_fill_manual(values = c(DIC = "#029C57", BIC = "#110C5E")) +
  scale_y_continuous(limits = c(0, 1),
                     labels = scales::percent_format(accuracy = 1)) +
  labs(x = NULL, y = "Poder empírico", fill = NULL,
       title = "Poder do teste por cenário de bloco") +
  theme_minimal(base_size = 12, base_family = "serif") +
  theme(legend.position = "top",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        plot.title       = element_text(hjust = 0.5),
        panel.border     = element_rect(color = "gray80", fill = NA, linewidth = 0.5))
```


## Escolher DIC ou BIC: poder por cenário de intensidade de bloco

```{r}
set.seed(123)
intensidades <- seq(0, 20, by = 1)

resultados <- lapply(intensidades, function(s) {
  sigma2_bloco <- s^2  # variância de bloco para esse s
  sigma2_erro  <- max(sigma2 - sigma2_bloco, 1)  # erro puro = total - bloco
  
  sim <- replicate(1000, {
    beta_orig <- rnorm(blocos, 0, sd = s)
    beta      <- beta_orig - mean(beta_orig)
    
    erro <- rnorm(n_obs, 0, sqrt(sigma2_erro))  # erro particionado
    y    <- mu + tau[trat] + beta[bloco] + erro
    
    p_bic <- summary(aov(y ~ factor(trat) + factor(bloco)))[[1]]["factor(trat)", "Pr(>F)"]
    p_dic <- summary(aov(y ~ factor(trat)))[[1]]["factor(trat)", "Pr(>F)"]
    
    c(bic = p_bic < 0.05, dic = p_dic < 0.05)
  })
  
  data.frame(
    intensidade = s,
    poder_dic   = mean(sim["dic", ]),
    poder_bic   = mean(sim["bic", ])
  )
})

df <- do.call(rbind, resultados)
```

```{r}
#| echo: false
df_long <- pivot_longer(df,
  cols      = c(poder_dic, poder_bic),
  names_to  = "modelo",
  values_to = "poder"
)

df_long$modelo <- factor(df_long$modelo, levels = c("poder_dic", "poder_bic"))

ggplot(df_long, aes(x = intensidade, y = poder, color = modelo, linetype = modelo)) +
  geom_line(linewidth = 1, alpha = 0.65) +
  geom_point(size = 2, alpha = 0.65) +
  geom_vline(xintercept = 3.5, linetype = "dashed", color = "gray60") +
  annotate("text", x = 3.8, y = 0.9,
           label = "ponto de cruzamento",
           hjust = 0, size = 3.5, color = "gray40") +
  scale_color_manual(
    values = c(poder_dic = "#029C57", poder_bic = "#110C5E"),
    labels = c("DIC", "BIC")
  ) +
  scale_linetype_manual(
    values = c(poder_dic = "dashed", poder_bic = "solid"),
    labels = c("DIC", "BIC")
  ) +
  scale_x_continuous(breaks = seq(0, 20, by = 2)) +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.1)) +
  labs(
    title    = "Poder empírico do DIC e BIC por intensidade do efeito de bloco",
    x        = "Intensidade do efeito de bloco (s)",
    y        = "Poder empírico",
    color    = NULL,
    linetype = NULL
  ) +
  theme_minimal(base_size = 13, base_family = "serif") +
  theme(legend.position = "top")
```

# Conclusão

Por meio das simulações a partir do determinado cenário, é possível concluir que a escolha entre os delineamentos DIC e O BIC depende diretamente da relevância dos blocos no experimento. Quando o efeito de bloco é irrelevante, o DIC apresenta poder ligeiramente superior — o BIC consome graus de liberdade sem benefício correspondente. Conforme a intensidade do bloco aumenta, o DIC perde poder progressivamente enquanto o BIC permanece estável, pois isola o efeito de bloco antes de testar os tratamentos. <br>

O ponto de cruzamento em s aproximadamente igual a 3.5 delimita a fronteira a partir da qual o BIC passa a ser preferível nesse caso, evidenciando que, em experimentos com heterogeneidade relevante entre unidades experimentais, o controle local é essencial para preservar o poder do teste. <br>


# Análise das observações
```{r, warning = FALSE, fig.width = 12, fig.height = 5}
library(patchwork)

sim_bforte2 <- 
  replicate(10, {
  erro  <- rnorm(n_obs, 0, sqrt(sigma2_erro_forte))
  y   <- mu + tau[trat] + beta_forte[bloco] + erro
  
})

# Pegar uma simulação
y_exemplo <- sim_bforte2[, 1]

# Tabela para ver qual é o ij de cada obs
df_obs <- data.frame(
  y     = y_exemplo,
  trat  = factor(trat),
  bloco = factor(bloco)
)

# Média por bloco
df_media <- aggregate(y ~ bloco, data = df_obs, FUN = mean)

g1<- ggplot(df_obs, aes(x = bloco, y = y)) +
  geom_point(aes(color = trat), size = 3) +
  geom_point(data = df_media, aes(x = bloco, y = y),
             shape = 8, size = 4, color = "black") +
  geom_hline(yintercept = mean(df_obs$y),
             linetype = "dashed", color = "gray50") +
  annotate("text", x = 0.6, y = mean(df_obs$y) + 1,
           label = "média geral", size = 3.0, color = "gray50") +
  scale_color_manual(values = c("pink", "yellow", "orange", "#C0392B"),
                     labels = paste("Tratamento", 1:4)) +
  labs(title = "Observações por bloco - forte",
       x     = "Blocos",
       y     = "y",
       color = NULL) +
  theme_minimal(base_size = 13, base_family = "serif") +
  theme(legend.position   = "top",
        plot.title        = element_text(hjust = 0.5),
        panel.grid.major  = element_blank(),
        panel.grid.minor  = element_blank(),
        panel.border      = element_rect(color = "gray80", fill = NA, linewidth = 0.5))



# Pegar uma simulação
y_exemplo2 <- sim_bforte2[, 4]

# Tabela para ver qual é o ij de cada obs
df_obs2 <- data.frame(
  y     = y_exemplo2,
  trat  = factor(trat),
  bloco = factor(bloco)
)

# Média por bloco
df_media <- aggregate(y ~ bloco, data = df_obs2, FUN = mean)

g2<- ggplot(df_obs2, aes(x = bloco, y = y)) +
  geom_point(aes(color = trat), size = 3) +
  geom_point(data = df_media, aes(x = bloco, y = y),
             shape = 8, size = 4, color = "black") +
  geom_hline(yintercept = mean(df_obs$y),
             linetype = "dashed", color = "gray50") +
  annotate("text", x = 0.6, y = mean(df_obs$y) + 1,
           label = "média geral", size = 3.0, color = "gray50") +
  scale_color_manual(values = c("pink", "yellow", "orange", "#C0392B"),
                     labels = paste("Tratamento", 1:4)) +
  labs(title = "Observações por bloco - forte",
       x     = "Blocos",
       y     = "y",
       color = NULL) +
  theme_minimal(base_size = 13, base_family = "serif") +
  theme(legend.position   = "top",
        plot.title        = element_text(hjust = 0.5),
        panel.grid.major  = element_blank(),
        panel.grid.minor  = element_blank(),
        panel.border      = element_rect(color = "gray80", fill = NA, linewidth = 0.5))


g1|g2
```

### por função:
```{r}
plot_bloco <- function(sim_matrix, col, titulo = "Observações por bloco") {
  y_exemplo <- sim_matrix[, col]
  
  df_obs <- data.frame(
    y     = y_exemplo,
    trat  = factor(trat),
    bloco = factor(bloco)
  )
  
  df_media <- aggregate(y ~ bloco, data = df_obs, FUN = mean)
  media_geral <- mean(df_obs$y)
  
  ggplot(df_obs, aes(x = bloco, y = y)) +
    geom_point(aes(color = trat), size = 3) +
    geom_point(data = df_media, aes(x = bloco, y = y),
               shape = 8, size = 4, color = "black") +
    geom_hline(yintercept = media_geral,
               linetype = "dashed", color = "gray50") +
    annotate("text", x = 0.6, y = media_geral + 1,
             label = "média geral", size = 3.0, color = "gray50") +
    scale_color_manual(values = c("pink", "yellow", "orange", "#C0392B"),
                       labels = paste("Tratamento", 1:4)) +
    labs(title = titulo,
         x     = "Blocos",
         y     = "y",
         color = NULL) +
    theme_minimal(base_size = 13, base_family = "serif") +
    theme(legend.position   = "top",
          plot.title        = element_text(hjust = 0.5),
          panel.grid.major  = element_blank(),
          panel.grid.minor  = element_blank(),
          panel.border      = element_rect(color = "gray80", fill = NA, linewidth = 0.5))
}
```

#### chama as colunas:
```{r, warning = FALSE, fig.width = 12, fig.height = 5}
g1 <- plot_bloco(sim_bforte2, col = 1, titulo = "Observações por bloco - forte")
g2 <- plot_bloco(sim_bforte2, col = 4, titulo = "Observações por bloco - forte")
g1 | g2
```
### testando uma funçao geral
```{r}
plot_bloco <- function(sim_matrix, col, titulo = "Observações por bloco", cores) {
  df_obs <- data.frame(
    y     = sim_matrix[, col],
    trat  = factor(trat),
    bloco = factor(bloco)
  )
  
  df_media    <- aggregate(y ~ bloco, data = df_obs, FUN = mean)
  media_geral <- mean(df_obs$y)
  
  ggplot(df_obs, aes(x = bloco, y = y)) +
    geom_point(aes(color = trat), size = 3) +
    geom_point(data = df_media, aes(x = bloco, y = y),
               shape = 8, size = 4, color = "black") +
    geom_hline(yintercept = media_geral,
               linetype = "dashed", color = "gray50") +
    annotate("text", x = 0.6, y = media_geral + 1,
             label = "média geral", size = 3.0, color = "gray50") +
    scale_color_manual(values = cores,
                       labels = paste("Tratamento", 1:4)) +
    labs(title = titulo, x = "Blocos", y = "y", color = NULL) +
    theme_minimal(base_size = 13, base_family = "serif") +
    theme(legend.position   = "top",
          plot.title        = element_text(hjust = 0.5),
          panel.grid.major  = element_blank(),
          panel.grid.minor  = element_blank(),
          panel.border      = element_rect(color = "gray80", fill = NA, linewidth = 0.5))
}
```

```{r, warning = FALSE, fig.width = 12, fig.height = 5}
cores_forte      <- c("pink", "yellow", "orange", "#C0392B")
cores_fraco      <- c("#110C5E", "#029C57", "#D85A30", "#C0392B")
cores_intermediario <- c("pink", "yellow", "orange", "#C0392B")  

# Forte
g1 <- plot_bloco(sim_bforte2,       col = 1, titulo = "Observações por bloco - forte",         cores = cores_forte)
g2 <- plot_bloco(sim_bforte2,       col = 4, titulo = "Observações por bloco - forte",         cores = cores_forte)
g3 <- plot_bloco(sim_bforte2,       col = 7, titulo = "Observações por bloco - forte",         cores = cores_forte)

# Fraco
g4 <- plot_bloco(sim_bfraco2,       col = 1, titulo = "Observações por bloco - fraco",         cores = cores_fraco)
g5 <- plot_bloco(sim_bfraco2,       col = 7, titulo = "Observações por bloco - fraco",         cores = cores_fraco)
g6 <- plot_bloco(sim_bfraco2,       col = 3, titulo = "Observações por bloco - fraco",         cores = cores_fraco)

g1|g2|g3
# Intermediário
#g7 <- plot_bloco(sim_bintermediario, col = 1, titulo = "Observações por bloco - intermediário", cores = cores_intermediario)
#g8 <- plot_bloco(sim_bintermediario, col = 4, titulo = "Observações por bloco - intermediário", cores = cores_intermediario)
#g9 <- plot_bloco(sim_bintermediario, col = 9, titulo = "Observações por bloco - intermediário", cores = cores_intermediario)
```


```{r}
sim_bfraco2 <- 
  replicate(10, {
  erro  <- rnorm(n_obs, 0, sqrt(sigma2_erro_fraco))
  y   <- mu + tau[trat] + beta_fraco[bloco] + erro
  
})


# Pegar uma simulação
y_exemplo3 <- sim_bfraco2[, 1]

# Tabela para ver qual é o ij de cada obs
df_obs3 <- data.frame(
  y     = y_exemplo3,
  trat  = factor(trat),
  bloco = factor(bloco)
)

# Média por tratamento
df_media <- aggregate(y ~ bloco, data = df_obs3, FUN = mean)

g3 <- ggplot(df_obs, aes(x = bloco, y = y)) +
  geom_point(aes(color = trat), size = 3) +
  geom_point(data = df_media, aes(x = bloco, y = y),
             shape = 8, size = 4, color = "black") +
  geom_hline(yintercept = mean(df_obs$y),
             linetype = "dashed", color = "gray50") +
  annotate("text", x = 0.6, y = mean(df_obs$y) + 1,
           label = "média geral", size = 3.0, color = "gray50") +
  scale_color_manual(values = c("#110C5E", "#029C57", "#D85A30", "#C0392B"),
                     labels = paste("Tratamento", 1:4)) +
  labs(title = "Observações por bloco - fraco",
       x     = "Blocos",
       y     = "y",
       color = NULL) +
  theme_minimal(base_size = 13, base_family = "serif") +
  theme(legend.position   = "top",
        plot.title        = element_text(hjust = 0.5),
        panel.grid.major  = element_blank(),
        panel.grid.minor  = element_blank(),
        panel.border      = element_rect(color = "gray80", fill = NA, linewidth = 0.5))


# Pegar uma simulação
y_exemplo4 <- sim_bfraco2[, 7]

# Tabela para ver qual é o ij de cada obs
df_obs4 <- data.frame(
  y     = y_exemplo3,
  trat  = factor(trat),
  bloco = factor(bloco)
)

# Média por tratamento
df_media <- aggregate(y ~ bloco, data = df_obs4, FUN = mean)

g4 <- ggplot(df_obs, aes(x = bloco, y = y)) +
  geom_point(aes(color = trat), size = 3) +
  geom_point(data = df_media, aes(x = bloco, y = y),
             shape = 8, size = 4, color = "black") +
  geom_hline(yintercept = mean(df_obs$y),
             linetype = "dashed", color = "gray50") +
  annotate("text", x = 0.6, y = mean(df_obs$y) + 1,
           label = "média geral", size = 3.0, color = "gray50") +
  scale_color_manual(values = c("#110C5E", "#029C57", "#D85A30", "#C0392B"),
                     labels = paste("Tratamento", 1:4)) +
  labs(title = "Observações por bloco - fraco",
       x     = "Blocos",
       y     = "y",
       color = NULL) +
  theme_minimal(base_size = 13, base_family = "serif") +
  theme(legend.position   = "top",
        plot.title        = element_text(hjust = 0.5),
        panel.grid.major  = element_blank(),
        panel.grid.minor  = element_blank(),
        panel.border      = element_rect(color = "gray80", fill = NA, linewidth = 0.5))

g3|g4
```





####### rever

```{r}
# Ajusta os modelos
fit_dic <- aov(y_exemplo ~ factor(trat))
fit_dbc <- aov(y_exemplo ~ factor(trat) + factor(bloco))

# Adiciona resíduos ao dataframe
df_obs$res_dic <- residuals(fit_dic)
df_obs$res_dbc <- residuals(fit_dbc)

# Transforma pra formato longo
df_res <- data.frame(
  residuo = c(df_obs$res_dic, df_obs$res_dbc),
  modelo  = rep(c("DIC", "DBC"), each = nrow(df_obs)),
  trat    = rep(df_obs$trat, 2),
  bloco   = rep(df_obs$bloco, 2)
)

ggplot(df_res, aes(x = trat, y = residuo, color = bloco)) +
  geom_point(size = 3) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  facet_wrap(~ modelo) +
  scale_color_manual(values = c("#110C5E", "#029C57", "#D85A30", "#C0392B"),
                     labels = paste("Bloco", 1:4)) +
  labs(title = "Resíduos por tratamento — DIC vs DBC (bloco forte)",
       x     = "Tratamento",
       y     = "Resíduo",
       color = NULL) +
  theme_minimal(base_size = 13, base_family = "serif") +
  theme(legend.position  = "top",
        plot.title       = element_text(hjust = 0.5),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border     = element_rect(color = "gray80", fill = NA, linewidth = 0.5))
```
